| Compétence        |           Coût           | Description                                                                |
| ----------------- | :----------------------: | -------------------------------------------------------------------------- |
| **Intimidant**    |          [1-5]           | Modificateur d'influence par la peur                                       |
| Parrain           |  8 (-1 par Intimidant)   | Les gens ne parleront pas de ce que vous avez fait ou dit                  |
| **Performance**   |          [1-5]           | Modificateur d'influence par la gestuelle                                  |
| Bouffon           |  8 (-1 par Performance)  | Possibilité de recevoir des pièces après avoir diverti (1d6 pa)            |
| **Persuasif**     |          [1-5]           | Modificateur d'influence par la parole                                     |
| Politicien        |   8 (-1 par Persuasif)   | Perception (Mentaliste) ne peut pas détecter vos intentions                |
| **Flatteur**      |          [1-5]           | Modificateur d'influence par les compliments                               |
| Dragueur          |   8 (-1 par Flatteur)    | Facilitent les relations et les révélations lors des discussions post-coït |
| **Charismatique** |          [1-5]           | Modificateur d'influence par la prestance                                  |
| Doyen             | 8 (-1 par Charismatique) | Les gens vanteront vos mérites, améliorant votre réputation locale         |
| **Veinard**       |            15            | Le joueur peut relancer ses échecs critiques                               |

---

## Veinard
>Nécessite 12 en Charisme

Permet de relancer 1d20 de son choix par session.

## Intimidant

[1-5] Modificateur d'influence par la peur.

### Parrain
>Nécessite Intimidant 5

Les gens ne parleront pas de ce que vous avez fait ou dit.

## Performance

[1-5] Modificateur d'influence par la gestuelle.

### Bouffon
>Nécessite Performance 5

Possibilité de recevoir des pièces après avoir diverti (1d6 pa).

## Persuasif

[1-5] Modificateur d'influence par la parole.

### Politicien
>Nécessite Persuasif 5

Le mentalisme ne peut pas détecter vos intentions.

## Flatteur

[1-5] Modificateur d'influence par les compliments

### Dragueur
>Nécessite Flatteur 5

Facilitent les relations et les révélations lors des discussions post-coït.

## Charismatique

[1-5] Modificateur d'influence par la prestance.

### Doyen
>Nécessite Charismatique 5

Les gens vanteront vos mérites, améliorant votre réputation locale.