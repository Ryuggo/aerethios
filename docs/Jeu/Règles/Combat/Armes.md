>Le type d'arme ainsi que ses matériaux vont influencer les bonus, malus et enchantements que celle-ci peut posséder.

---

## Poids
>If you wear armor that you lack proficiency with, you have disadvantage on any ability check, saving throw, or Attack roll that involves Strength or Dexterity, and you can’t cast Spells.

### Courte
>Une arme courte ne peut pas être maniée avec 2 mains.

>Un objet qui ne ressemble pas à une arme ne fait que 1d4 de dégâts

| Arme    |             Prix |     Dégâts     | Matériaux |      Poids      |
| ------- | ---------------: | :------------: | :-------: | :-------------: |
| Poings  |                  | 1d4 contondant |           |                 |
| Objet   |                  |      1d4       |           |                 |
| Caetus  |   5 pa<br>*5 gp* |                |           | 1kg<br>*2 lb.*  |
| Dague   | 10 pa<br>*10 gp* |                |           | 2 kg<br>*4 lb.* |
| Griffes |                  |                |           |                 |
**Effets**
    -

### 1 main
>Une arme à 1 main ne peut pas être maniée avec 2 mains.

| Arme |           Prix | Dégâts | Matériaux |      Poids      |
| ---- | -------------: | :----: | :-------: | :-------------: |
|      | 5 pa<br>*5 gp* |        |           | 1 kg<br>*3 lb.* |
**Effets**
    -

### 2 mains
>Une arme à 2 mains peut être maniée avec 1 main sans malus si elle est Versatile.
>Sinon le joueur devra utiliser sa force et aura un désavantage s'il n'a pas la compétence pour être efficace ainsi.

| Arme |                Prix | Dégâts | Matériaux | Propriété |       Poids       |
| ---- | ------------------: | :----: | :-------: | --------- | :---------------: |
|      | 7 50 pa<br>*750 gp* |        |           |           | 20 kg<br>*40 lb.* |
**Effets**
    -

### Haste

| Arme |             Prix | Dégâts | Force | Matériaux |       Poids       |
| ---- | ---------------: | :----: | :---: | :-------: | :---------------: |
|      | 30 pa<br>*30 gp* |        |  ---  |           | 20 kg<br>*40 lb.* |
**Effets**
    -

### Distance

| Arme |           Prix | Dégâts | Force | Matériaux |      Poids      |
| ---- | -------------: | :----: | :---: | :-------: | :-------------: |
|      | 5 pa<br>*5 gp* |        |  ---  |           | 1 kg<br>*2 lb.* |
**Effets**
    -

---

## Matériaux